...
import { ClientKafka, MessagePattern, Payload, Ctx, KafkaContext } from '@nestjs/microservices';
...
export class KafkaService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    console.log('[KafkaService] Manually subscribing to reply topics for message patterns...');
    ...
    console.log('[KafkaService] DEBUG: Consumer exists:', !!this.kafkaClient.consumer);
  ...
  async sendMessage(topic: string, message: any): Promise<any> {
    try {
      ...
      console.log(`[KafkaService] Using emit() with correlationId: ${correlationId}`);
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          if (this.pendingRequests.has(correlationId)) {
            ...
            reject(new Error(`Request timeout after ${this.REQUEST_TIMEOUT}ms for correlation ID: ${correlationId}`));
          }
        }, this.REQUEST_TIMEOUT);

        // Emit the message
        this.kafkaClient.emit(topic, messageWithId);
        console.log(`[KafkaService] Emitted message with correlation ID: ${correlationId}, waiting for reply...`);
      });
    } catch (error) {
  ...
  public handleReply(topic: string, message: any, context: KafkaContext) {
    ...
    if (kafkaMessage.key) {
      ...
      console.log(`[KafkaService] Found correlation ID in message key: ${correlationId}`);
    }
    ...
    if (!correlationId && kafkaMessage.headers) {
      ...
      if (headers['kafka_correlationId']) {
        ...
        console.log(`[KafkaService] Found correlation ID in headers: ${correlationId}`);
      }
    }

    // If still not found, try payload
    if (!correlationId && message && typeof message === 'object' && message.__correlationId) {
      correlationId = message.__correlationId;
      console.log(`[KafkaService] Found correlation ID in payload: ${correlationId}`);
    }

    if (!correlationId) {
      console.error(`[KafkaService] No correlation ID found in reply from topic: ${topic}`);
      return;
    }
    ...
    if (pendingRequest) {
      console.log(`[KafkaService] Resolving pending request for correlation ID: ${correlationId}`);
      this.pendingRequests.delete(correlationId);
      pendingRequest.resolve(message);
    } else {
      console.warn(`[KafkaService] No pending request found for correlation ID: ${correlationId}`);
    }
  }
  ...
  private cleanupExpiredRequests() {
    ...
    for (const [correlationId, request] of this.pendingRequests.entries()) {
      if (now - request.timestamp > this.REQUEST_TIMEOUT) {
        ...
        request.reject(new Error(`Request expired for correlation ID: ${correlationId}`));
      }
    }
    ...
    if (expiredIds.length > 0) {
      console.log(`[KafkaService] Cleaned up ${expiredIds.length} expired requests`);
    }
  }
}
